...
func (db *Database) GetExternalAssessments(user User) []DigitalDexterityAssessment {
	...
	assessments := make([]DigitalDexterityAssessment, len(externalAssessments))
	for idx, assessment := range externalAssessments {
		assessments[idx] = assessment.DigitalDexterityAssessment
	}

	return assessments
}

var ErrNoSelfAssessments = errors.New("there is no self assessment")
...
func (db *Database) GetSelfAssessment(user User) (*SelfAssessment, error) {
	...
	if assessment.UserID != user.ID {
		return nil, ErrNoSelfAssessments
	}
